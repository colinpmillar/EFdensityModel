...
A large electrofishing dataset is statistically complex to model because the sampleing process is multilevel. In a sampling scheme where there are sevaral types of fish sampled on each fishing event (ie species and lifestages), then there are conseivably the following type of random variation present

\begin{itemize}
  \item between fish
  \item between fish type
  \item between pass
  \item between sample
\end{itemize}

Capture probability can vary with all these things, while fish density should only vary between fish-type and between sample.
...
Huggins showed that if onr conditions on only those fish captured, then the conditional probability of capture is
\begin{align}
...
\end{align}
where $\alpha_{jm}$ is multivariate normal and where the expectation can vary by fish, or by fish and fishing pass.  Such a model would lead to an overdispersed multinomial distribuion.  There are a variaty of ways to include fish to fish random variation
