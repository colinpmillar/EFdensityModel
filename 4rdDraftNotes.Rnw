...
<<settings, echo=FALSE, meesage=FALSE>>=
...
pdata <- expand.grid(pass = 1:3,
                     ...
                     species = c("Trout", "Salmon"),
                     sample = 1:n)
pdata$id <- with(pdata, as.numeric(factor(paste(lifestage, species, sample))))
...
<<boot_nu, cache=TRUE, message=FALSE>>=
