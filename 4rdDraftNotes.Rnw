...
<<fitting_p, cache=TRUE>>=
library(rstan)
pmod <- ef::efp(y ~ lifestage + species, pass = pass, data = pdata, hessian = TRUE)
@

And now we can fit a density model to each sample

<<fitting_nu>>=
nmod <- glm(y ~ lifestage:species:factor(sample)-1, family = poisson, data = ndata, offset = log(ndata$pi))
@

Now we can calculate $R_i$ for each nu
